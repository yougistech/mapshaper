...
export var ExportControl = function(gui) {
  ...
  function initSelectAll() {
    var toggleHtml = '<label><input type="checkbox" value="toggle" checked> 选择所有</label>';
  ...
  function initLayerMenu() {
    ...
    // update menu title
    gui.container.findChild('.export-layers .menu-title').html(layers.length == 1 ? '图层名称' : '图层列表');
