...
export function Popup(gui, toNext, toPrev) {
  ...
  function renderAddFieldPopup(recId, lyr) {
    var popup = showPopupAlert('', '添加字段'/**'Add field'**/);
    var el = popup.container();
    el.addClass('option-menu');
    var html = `<div><input type="text" class="field-name text-input" placeholder="字段名称"></div>
    <div><input type="text" class="field-value text-input" placeholder="值"><div>
    <div tabindex="0" class="btn dialog-btn">应用</div> <span class="inline-checkbox"><input type="checkbox" class="all" />为所有记录赋值</span>`;
    el.html(html);
